...
订阅由 S-UI 的 `subDomain` + `subPath` 生成，不要把具体订阅 URL 或客户端名写进仓库。分流改 `clash-ext.yaml` 即可。S-UI 每次生成订阅都会读数据库，**同步规则不会重启 sing-box**。
...
SSH 主机和用户不要写进仓库。本机任选其一：

- 环境变量 `SUI_HOST`、`SUI_USER`（可选 `SUI_SSH_KEY`）
- 复制 [`scripts/deploy.env.example`](scripts/deploy.env.example) 为 `.deploy/config`（已 gitignore）
...
| `SUI_HOST` | SSH 主机 |
| `SUI_USER` | SSH 用户 |
| `SUI_SSH_KEY` | 部署用私钥全文（本机可放 `.deploy/id_ed25519`，不要提交） |
