...
- 把面板密码、UUID、Reality 私钥提交进 git
- 重启 s-ui / sing-box 来刷新规则（没必要）

## 主站

- SSH：`ubuntu@13.196.71.52`（passwordless sudo）
- 订阅：`https://clash.lobe.li/sub/<name>?format=clash`
- 客户端名包括 `canisminor`、`lobehub` 等
- 数据库：`/usr/local/s-ui/db/s-ui.db` 的 `settings.key=subClashExt`
